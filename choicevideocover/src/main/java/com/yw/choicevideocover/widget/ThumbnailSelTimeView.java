...
public class ThumbnailSelTimeView extends View {
    ...
    private void init() {
        ...
        rectWidth = (int) getResources().getDimension(R.dimen.dp40);
    ...
    public float getRectLeft() {
        return (float) rectF.left / (float) mWidth;
    }
    ...
    public float getRadius() {
        return (float) rectF.right - rectWidth / 2 / (float) mWidth;
    }
...
//                printRectF(event.getX());
    ...
    public float getPointX() {
        return this.rectF.left + rectWidth / 2;
    }

    public float getRealRectLeft() {
        return this.rectF.left;
    }


    private void printRectF(float pointer) {
        Log.e("看看这个矩形的情况：", "left:" + rectF.left + ", right:" + rectF.right + ", top:" + rectF.top + ", bottom:" + rectF.bottom);
        setCusRectF(pointer);
    }
     ...
     * 外部设置需要移动的位置
     */
    public void setCusRectF(float pointer) {
        this.rectF.left = pointer - rectWidth / 2;
        this.rectF.right = this.rectF.left + rectWidth;
        invalidate();
        if (onScrollBorderListener != null) {
            onScrollBorderListener.onScrollStateChange();
        }
    ...
    protected void onDraw(Canvas canvas) {
        mPaint.setColor(Color.parseColor("#FE6800"));
        mPaint.setStyle(Paint.Style.STROKE);
